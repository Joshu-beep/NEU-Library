...
window.addEventListener("load", function () {
  ...
  startBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      video.onloadedmetadata = function () {
        ...
        setStatus(
          "ok",
          "📷 Camera active — hold your QR code up to the camera",
        );
        tick();
      };
    } catch (err) {
      ...
      if (
        ...
        setStatus(
          "error",
          "🚫 Camera permission denied. Please tap the camera icon in your browser address bar and allow access, then refresh.",
        );
  ...
  async function handleQR(raw) {
    ...
    if (!payload || !payload.uid) {
      setStatus(
        "error",
        "❌ Invalid QR code. Please use your NEU Library QR code.",
      );
      resume(3000);
      return;
    }
    ...
    if (user.is_blocked) {
      setStatus(
        "error",
        "🚫 Access denied — your account has been blocked. Contact the library admin.",
      );
      resume(4000);
      return;
    }
    ...
    // Show user card
    scannedUid = user.id;
    document.getElementById("uInitial").textContent = (user.name ||
      "?")[0].toUpperCase();
    ...
    if (logResult.data) {
      ...
      setStatus(
        "ok",
        "✅ " + user.name + " has been logged out. See you next time!",
      );
      reset(3000);
      return;
    } else {
      ...
      setStatus(
        "ok",
        "✓ " + user.name + " identified — select a reason and check in",
      );
      refreshBtn();
    }
  }
  ...
  checkinBtn.addEventListener("click", async function () {
    ...
    setStatus(
      "ok",
      "✅ " + name + ' checked in for "' + reason + '". Have a productive day!',
    );
    reset(4000);
  });
